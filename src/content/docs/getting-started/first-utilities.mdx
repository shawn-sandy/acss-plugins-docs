...
<Aside type="caution">
Import order matters. `token-bridge.css` defines the CSS custom properties that `utilities.css` references. If utilities loads first, the `var()` fallbacks kick in rather than your theme values.
</Aside>
...
→ See [/utility-tune](/acss-plugins-docs/acss-utilities/commands/utility-tune) for the full reference.
