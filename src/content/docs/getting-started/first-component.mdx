...
<Aside type="tip">
  If `ui.tsx` already exists in your components folder, it won't be overwritten
  — `/kit-add` always skips existing files rather than clobber them.
</Aside>
...
<Aside type="note">
  The `--btn-*` tokens are scoped to `.btn`. You can override them globally, per
  page, or per component wrapper — the full CSS cascade applies.
</Aside>
