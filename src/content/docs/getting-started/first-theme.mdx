...
`/theme-create` takes one hex color — your "seed" — and generates 15 required semantic roles using OKLCH color math. Every light and dark value is tested against WCAG 2.2 AA contrast requirements before the files are written.

The generated palette covers the 15 required roles:
...
Three optional roles (`surface-subtle`, `text-subtle`, `brand-accent`) are recognized by the token pipeline but are not generated — add them manually or via a `/theme-from-design` import.
...
3. If any contrast pair fails, Claude adjusts the lightness value in OKLCH space and retries. The files are only written once all 12 pairs pass.
