...
7. **Emit refactored source.** Produce a clean version of the original input with the inline style removed:
   ...
   - **In-place editing (IDE selection).** When the source file path and line range were captured in step 1, use `Edit` to replace the **element span** in the source file with the refactored version — no manual copy-paste needed. Use the correct replacement target based on what step 1 captured: if the IDE selection was a full element, replace that selected text; if the selection was partial (attribute value only) and step 1 expanded it via `Read`, replace the full expanded element (not the original selection text) so the `style` attribute is correctly removed and the `class` reference is added without corrupting surrounding markup. If the element text appears more than once in the file, fall back to emitting the refactored source to chat and note which file and line to update.
