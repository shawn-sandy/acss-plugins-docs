...
1. **Parse input.** Detect the input form:
   ...
   - **JSX style object** — match `style={{...}}`; extract the object literal body; split key/value pairs. Convert each camelCase key to kebab-case. For numeric literal values, emit a "verify unit" warning. For JS expression values, emit a `/* unresolved: <expr> */` placeholder.
