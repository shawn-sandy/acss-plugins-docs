...
Generates a 10-step OKLCH color scale (steps 50–900) from a seed hex color. Used by the [`/color-scale`](/acss-plugins-docs/acss-kit/commands/color-scale) command.
...
**Output (stdout) — JSON shape (`--format=json` or `--format=both`):**
...
Each `steps` entry includes a `clamped` boolean and a `reasons` array that indicate whether gamut clamping occurred at that lightness level. The top-level `reasons` array collects all per-step clamp messages.

**Output (stdout) — CSS block (`--format=css` or `--format=both`):**
...
When `--format=both`, JSON is emitted first, followed by a blank line, then the CSS block.

**Exit codes:** 0 = success, 2 = invalid input (bad hex, bad name, or unknown `--format`)
