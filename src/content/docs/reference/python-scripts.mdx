...
Most scripts use **Python 3 stdlib only** — no pip dependencies. The one exception is `validate_utilities.py`, which requires `tinycss2` (a test dependency already present in the repo). They follow one of two contract families.
...
Walks a priority-ordered list of candidate CSS/SCSS entry files in the project to find where `/setup` should append `@import` lines for `light.css`, `dark.css`, `token-bridge.css`, and `utilities.css`. Also scans each candidate to count how many of those artifact imports already exist, so the wire-up step is idempotent.
...
`source` values: `"detected"` (at least one candidate found), `"none"` (no entry file located).

**Exit codes:** 0 = at least one candidate found, 1 = no candidate found.
...
Rewrites colon-based utility class names (v0.1.x) to hyphen-based names (v0.2.0) across source files — e.g. `sm:hide` → `sm-hide`, `md:p-6` → `md-p-6`. Without `--write` it performs a dry run and prints a unified diff.

**Inputs:**

```bash
python3 migrate_classnames.py <paths...> [--write] [--include=<globs>] [--exclude=<globs>] [--prefixes=<list>]
```

| Flag         | Default             | Description                                                  |
| ------------ | ------------------- | ------------------------------------------------------------ |
| `paths`      | required            | Files or directories to process (multiple allowed)           |
| `--write`    | false (dry run)     | Apply changes in place instead of printing a diff            |
| `--include`  | all files           | Comma-separated glob patterns to include (e.g. `*.tsx,*.ts`) |
| `--exclude`  | none                | Comma-separated glob patterns to exclude                     |
| `--prefixes` | `sm,md,lg,xl,print` | Breakpoint prefixes to rewrite                               |

**Handled file types:**

- **JSX/TSX, JS/TS** — rewrites inside `className` attributes and `clsx`/`classnames` calls
- **HTML/Svelte/Vue/Astro** — rewrites inside `class` and `className` attributes
- **CSS/SCSS** — rewrites inside `@apply` directives and escaped-colon selectors (`.sm\:hide`)

**Output (dry run, stdout):** Unified diff per modified file; stderr: `# Dry run complete — rerun with --write to apply changes.`

**Output (`--write`, stdout):** `updated: {path}` per file written; errors to stderr.

**Exit codes:**

- 0 — no changes needed, or all changes applied successfully (`--write`)
- 1 — dry run: changes pending (rerun with `--write` to apply)
- 2 — usage error, IO error, or path not found

<Aside type="caution">
  Combo prefixes like `lg:hover:bg-primary` will partially rewrite incorrectly
  in v0.2.0. Vue `:class` bindings, Svelte `class:` directives, and Astro
  expressions inside `{}` are also not rewritten.
</Aside>
