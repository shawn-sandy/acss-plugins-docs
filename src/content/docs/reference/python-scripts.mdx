...
description: API reference for every Python script in acss-kit and style-agent — inputs, outputs, exit codes, and contract families.
---
...
<ColorSwatch roles={["primary"]} table={false} />
...
<Aside type="danger">
  The two scripts in this section — `detect_html_target.py` and
  `verify_html_integration.py` — were **removed from upstream** alongside the
  `/kit-add-html` command and `components-html` skill (commit `7827553`). The
  HTML-target workflow no longer exists as a separate path; use
  [`/kit-add`](/acss-plugins-docs/acss-kit/commands/kit-add) for all component
  generation going forward. These reference entries are preserved for projects
  that installed the HTML workflow before it was removed.
</Aside>

#### `detect_html_target.py`

**Contract:** Detector  
**Location:** `plugins/acss-kit/scripts/detect_html_target.py` _(removed upstream)_
...
**Contract:** Detector  
**Location:** `plugins/acss-kit/scripts/verify_html_integration.py` _(removed upstream)_
...
## acss-kit utility scripts

All utility scripts moved to `plugins/acss-kit/scripts/` in acss-kit v1.0.0
when the utility commands were absorbed from the now-deprecated
`acss-utilities` plugin.

### `generate_utilities.py`

**Contract:** Generator  
**Location:** `plugins/acss-kit/scripts/generate_utilities.py`
...
**Contract:** Validator  
**Location:** `plugins/acss-kit/scripts/validate_utilities.py`
...
**Contract:** Generator  
**Location:** `plugins/acss-kit/scripts/generate_bridge.py`

Generates `token-bridge.css` — the alias layer that maps fpkit-style utility
names (`--color-error`, `--color-primary-light`, etc.) to acss-kit
`--color-*` theme roles. Run automatically by `/theme-create` and
`/theme-update` when a `utilitiesDir` is configured; call manually via
`/utility-bridge` to regenerate after editing theme files directly.

**Inputs:**

```bash
python3 generate_bridge.py --theme-dir=<dir> --out=<path>
```

| Flag          | Description                                      |
| ------------- | ------------------------------------------------ |
| `--theme-dir` | Directory containing `light.css` / `dark.css`    |
| `--out`       | Output path for the generated `token-bridge.css` |

**Output:** `token-bridge.css` with `:root` and `[data-theme="dark"]` blocks.

**Exit codes:** 0 = success, 2 = IO/usage error

---

### `detect_target.py` (utilities mode)

**Contract:** Detector  
**Location:** `plugins/acss-kit/scripts/detect_target.py`

The unified target detector handles both component and utility detection.
Pass `--what=utilities` to resolve the utility output directory.

**Inputs:**

```bash
python3 detect_target.py --what=utilities [project_root]
```
...
**Location:** `plugins/acss-kit/scripts/migrate_classnames.py`

Migrates utility class names from the 0.1.x colon form (`sm:hide`, `md:p-6`) to the 0.2.0 hyphen form (`sm-hide`, `md-p-6`) in source files. By default performs a **dry run** and prints a unified diff per modified file. Pass `--write` to apply changes in place.
...
<Aside type="note">
  `migrate_classnames.py` is a one-time migration tool for projects that were
  set up with acss-utilities 0.1.x. If your project was set up with 0.2.0 or
  later, you do not need to run it.
</Aside>

---

## DESIGN.md / Figma bridge scripts

The following scripts were added in acss-kit v1.3.1 (Workstream A/C) to support the `/design-export`, `/theme-from-design`, and `/theme-from-figma` commands. All follow the **Generator/Validator contract** unless noted.

### `tokens_to_design_md.py`

**Contract:** Generator
**Location:** `plugins/acss-kit/scripts/tokens_to_design_md.py`

Converts the acss-kit theme tokens JSON to a [DESIGN.md](https://github.com/google-labs-code/design.md) file. This is the inverse of `design_md_to_tokens.py` and provides the import-_into_-DESIGN.md direction the upstream CLI lacks.

**Inputs:**

```bash
python3 tokens_to_design_md.py <tokens.json> [--name=<Brand>]
python3 tokens_to_design_md.py --stdin [--name=<Brand>]
python3 tokens_to_design_md.py --dir=<dir> [--name=<Brand>]  # shells css_to_tokens
python3 tokens_to_design_md.py --self-test
```

| Flag             | Description                                                           |
| ---------------- | --------------------------------------------------------------------- |
| `<tokens.json>`  | Path to a `theme.tokens.json` file                                    |
| `--stdin`        | Read tokens JSON from stdin                                           |
| `--dir=<dir>`    | Directory with `light.css` etc.; shells `css_to_tokens.py` internally |
| `--name=<Brand>` | Brand name for DESIGN.md front-matter                                 |
| `--self-test`    | Run internal round-trip fixture and exit                              |

**Output (stdout):** DESIGN.md text (YAML front-matter + markdown body).

**Round-trip semantics:** Semantic, not byte-identical. The 18 `--color-*` roles are emitted under DESIGN.md token names; M3 ladder tokens not modelled by acss-kit are not reproduced. An exported DESIGN.md re-imports correctly via `design_md_to_tokens.py`.

**Exit codes:** 0 = success, 1 = logical failure (nothing to emit), 2 = IO/usage error
...
**Contract:** Generator
**Location:** `plugins/acss-kit/scripts/design_md_to_tokens.py`

Converts a [DESIGN.md](https://github.com/google-labs-code/design.md) to the acss-kit theme tokens JSON. Called by the `/theme-from-design` command. Shells `npx @google/design.md export --format css-tailwind` to parse the DESIGN.md, then maps Tailwind v4 / Material-3 `@theme` property names to acss-kit's 18 color roles, spacing, rounded, and typography scales. Color roles absent from the DESIGN.md are synthesized via OKLCH gap-fill.

**Inputs:**

```bash
python3 design_md_to_tokens.py <DESIGN.md>
python3 design_md_to_tokens.py --stdin      # reads a css-tailwind block from stdin
python3 design_md_to_tokens.py --self-test
```

**Output (stdout):** `theme.tokens.json` consumed by `tokens_to_css.py`.

**Exit codes:** 0 = success, 1 = logical failure (e.g. no resolvable primary; reasons to stderr), 2 = IO/usage error

<Aside type="caution">
  This script shells `npx @google/design.md export --format css-tailwind` and
  requires Node/`npx`. The Tailwind v4 `@theme` prefix shape emitted by the
  upstream CLI is `alpha` — see the `COLOR_SOURCES` / `TW_SPACE` / `TW_RADIUS` /
  `TW_TYPO_*` tables in the script for the single reconciliation point.
</Aside>
...
**Contract:** Generator
**Location:** `plugins/acss-kit/scripts/figma_to_tokens.py`

Converts the Figma MCP server's `get_variable_defs` output to the acss-kit theme tokens JSON. Called by the `/theme-from-figma` command. Normalizes Figma's `category/name` variable format into the same `@theme`-style form expected by `design_md_to_tokens.build_tokens`, reusing the full M3-to-roles mapping and OKLCH synthesis with no second mapping table.

**Inputs:**

```bash
python3 figma_to_tokens.py <vars.json>   # get_variable_defs JSON map
python3 figma_to_tokens.py --stdin
python3 figma_to_tokens.py --self-test
```

| Input         | Description                                                                           |
| ------------- | ------------------------------------------------------------------------------------- |
| `<vars.json>` | JSON file containing a flat `{ "category/name": value }` map from `get_variable_defs` |
| `--stdin`     | Read the JSON map from stdin                                                          |
| `--self-test` | Run internal fixture and exit                                                         |
...
<Aside type="note">
  The only Figma-specific assumption is the `FIGMA_CATEGORIES` table (Figma
  variable category → css-tailwind prefix). This is the single reconciliation
  point against real `get_variable_defs` output.
</Aside>
...
**Contract:** Detector
**Location:** `plugins/acss-kit/scripts/validate_design_md.py`

Validates a DESIGN.md before the token pipeline consumes it. Called automatically by `/theme-from-design` before conversion. Shells `npx @google/design.md lint <file> --format json` and normalizes findings to the Detector contract.

**Key deviation from upstream:** A **missing primary color is a hard error** (exit 1) even though the upstream CLI treats it as a warning, because the primary is the OKLCH seed the entire pipeline depends on.

**Inputs:**

```bash
python3 validate_design_md.py <DESIGN.md>
python3 validate_design_md.py --self-test
```
...
```json
{
  "valid": true,
  "errors": [],
  "warnings": ["Missing optional section: spacing"],
  "reasons": ["DESIGN.md is valid — primary color present"]
}
```

**Exit codes:** 0 = usable, 1 = rejected (hard errors), 2 = IO/usage error
...
**Contract:** Validator
**Location:** `plugins/acss-kit/scripts/validate_tokens.py`

Validates the non-color sections of a `theme.tokens.json` — spacing, rounded, and typography — for structural correctness. The dimension/typography equivalent of the WCAG contrast gate that `validate_theme.py` applies to colors.

**Checks:**

- `spacing` / `rounded`: every value is a valid dimension (`0` or `<num>px|em|rem`)
- `spacing` / `rounded`: recommended scale steps present (`sm`, `md`, `lg` for spacing; `md` for rounded) — warning if missing
- `typography`: `size` is a dimension; `weight` is numeric or keyword; `line` is unitless or dimension; `tracking` is a dimension; `family` is a non-empty string

**Inputs:**

```bash
python3 validate_tokens.py <tokens.json>
python3 validate_tokens.py --stdin
python3 validate_tokens.py --self-test
```

**Exit codes:** 0 = no errors (warnings allowed), 1 = at least one structural error, 2 = IO/usage error
