---
title: Core Concepts
description: The mental model behind acss-kit — generated code vs npm packages, the UI foundation, data-* variants, CSS custom properties, and dependency resolution.
---
...
`@fpkit/acss` is a real npm package. acss-kit deliberately does not install it. Instead, Claude reads fpkit component patterns from bundled reference documents and writes self-contained TypeScript + SCSS files directly into your project. You own the output — you can edit it freely, and there is nothing to update via npm.

The trade-off is that updates require re-running `/kit-add` (which respects existing files; see [Bottom-up generation and skip-existing](#bottom-up-generation-and-skip-existing) below).
...
Generated components use `data-*` attribute selectors for styling variants instead of BEM modifier classes.

```tsx
<Button data-btn="lg" data-color="primary">
  Submit
</Button>
```
...
.btn {
  &[data-btn~="lg"] {
    font-size: var(--btn-lg-fs, 1.125rem);
  }
  &[data-color="primary"] {
...
<ComponentPreview
  code={`<button class="btn" data-btn="lg" data-color="primary">Submit</button>`}
/>
...
<Aside type="tip">
  Run `/css-variables` to browse the full token list, or see the [CSS Variables
  Reference](/acss-plugins-docs/acss-kit/css-variables) page for naming
  conventions and approved abbreviations.
</Aside>
...
<Aside type="note">
  Run `/kit-list <component>` before generating to see a component's full
  dependency tree without writing any files.
</Aside>

## .acss-target.json — project config

On the first `/kit-add` run in a project, the plugin writes a `.acss-target.json` file at the project root. After v0.5.0 it also captures a `stack` block describing the build tool, CSS pipeline, and entrypoint so subsequent runs can tailor advice:
...
- [Your First Component](/acss-plugins-docs/getting-started/first-component) — hands-on walkthrough
- [acss-kit Overview](/acss-plugins-docs/acss-kit/overview) — commands and skills at a glance
- [CSS Variables Reference](/acss-plugins-docs/acss-kit/css-variables) — full token list and naming conventions
- [`/kit-add` command](/acss-plugins-docs/acss-kit/commands/kit-add) — generation command reference
