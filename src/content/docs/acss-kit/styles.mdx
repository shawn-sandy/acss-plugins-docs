...
description: How to use the acss-kit theming system — OKLCH light/dark themes, brand presets, theme-role edits, and import order.
---
...
A theme defines 18 `--color-*` roles (15 required + 3 optional), grouped by purpose. Override any role in your theme file and every component that reads it restyles immediately.

| Category         | Role tokens                                                                                                        | Purpose                                                     |
| ---------------- | ------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------- |
...
<Aside type="tip">
  Run `/color-scale <hex>` to preview a full 10-step OKLCH palette for a hex
  color before committing it as a theme seed. The output is available in CSS,
  JSON, or both.
</Aside>

## Related

- [Component usage guides](/acss-plugins-docs/acss-kit/components) — components that consume these roles
- [OKLCH Theming deep-dive](/acss-plugins-docs/acss-kit/oklch-theming) — OKLCH math and palette mechanics
- [CSS Variables Reference](/acss-plugins-docs/acss-kit/css-variables) — full token list
- [Utilities consumer guide](/acss-plugins-docs/acss-kit/utilities) — atomic CSS utility classes
- Styles skill (maintainer reference): [`/kit-add` command](/acss-plugins-docs/acss-kit/commands/kit-add)
