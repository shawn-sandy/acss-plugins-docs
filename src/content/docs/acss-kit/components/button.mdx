...
description: Consumer guide for the acss-kit Button component — the primary interactive element with size, style, and color variants applied via data-* attributes.
---
...
<ComponentPreview
  ...
  <button style="padding:0.5em 1.25em;border-radius:0.375rem;border:none;background:var(--color-danger,#c2410c);color:#fff;font-weight:600;cursor:pointer;">Danger</button>
  ...
  <button style="padding:0.5em 1.5em;border-radius:9999px;border:none;background:var(--color-primary,#6d28d9);color:#fff;font-weight:600;cursor:pointer;">Pill</button>
</div>`}
...
2. **Add this component:** `/kit-add Button` — copies `button.tsx` + `button.scss` into your components directory (default `src/components/fpkit/`). Or run `/kit-sync` to install all components at once.

</Steps>

Button has no upstream dependencies. It is self-contained — no `@fpkit/acss` install required.
...
<ColorSwatch
  roles={["primary", "danger", "success", "warning"]}
  table={false}
/>
...
- [Icon Button](/acss-plugins-docs/acss-kit/components/icon-button) — icon-only variant built on Button
- [`component-button` skill](/acss-plugins-docs/acss-kit/skills/component-button) — maintainer reference (generation contract)
