...
description: Consumer guide for the acss-kit Popover component — native HTML Popover API with auto/manual modes, arrow, and controlled open state.
---
...
<Aside type="note">
  **Browser support:** Chrome/Edge 125+, Safari 17.4+, Firefox 125+. In browsers
  ...
  for graceful degradation or feature-detect
  `'showPopover' in HTMLElement.prototype`.

**Baseline status: Newly available — not yet Widely available.** If your
project uses a `baseline widely available` browser target (what `/setup`
writes), this component does not yet meet that bar. Feature-detect rather than
dropping it: `@supports not selector(:popover-open)` in CSS is the recommended
approach. Confirm current status at
[webstatus.dev](https://webstatus.dev) — this feature's Baseline date [was
corrected once after publication](https://web.dev/blog/popover-baseline).

</Aside>

<ComponentPreview
  code={`<div style="display:inline-flex;flex-direction:column;align-items:flex-start;gap:0.5rem;position:relative;">
  <button type="button" style="padding:0.5em 1em;border-radius:0.375rem;border:1px solid var(--color-border-strong,#94a3b8);background:var(--color-surface,#f8fafc);color:var(--color-text,#1e293b);cursor:pointer;font-size:0.875rem;">More info ▾</button>
  <div style="background:var(--color-background,#fff);border:1px solid var(--color-border,#e2e8f0);border-radius:0.5rem;padding:0.875rem 1rem;box-shadow:0 4px 16px rgba(0,0,0,0.12);max-width:18rem;font-size:0.9375rem;color:var(--color-text,#1e293b);">
    Additional context shown in a popover. Dismiss by clicking outside or pressing Escape.
  </div>
</div>`}
...
2. Add the component: `/kit-add Popover` — copies `popover.tsx` + `popover.scss` into your components directory (default `src/components/fpkit/`). Or run `/kit-sync` to install all components at once.
...
The trigger, popover panel, arrow, and close button are all rendered by the single `Popover` component. Adjust the path to match the `componentsDir` in your `.acss-target.json`.
...
- [`component-popover` skill](/acss-plugins-docs/acss-kit/skills/component-popover) — AI generation workflow reference
