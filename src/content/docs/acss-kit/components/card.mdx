...
description: Consumer guide for the acss-kit Card compound component — flexible content container with header, body, and footer regions, plus an interactive whole-card variant.
---
...
<ComponentPreview
  code={`<div style="border:1px solid var(--color-border,#e4e4e7);border-radius:0.75rem;box-shadow:0 2px 8px rgba(0,0,0,0.06);overflow:hidden;max-width:22rem;">
  <div style="padding:1rem 1.25rem;border-bottom:1px solid var(--color-border,#e4e4e7);"><h3 style="margin:0;font-size:1rem;font-weight:600;">Product Name</h3></div>
  <div style="padding:1rem 1.25rem;font-size:0.875rem;color:var(--color-text-muted,#71717a);">Product description here…</div>
  <div style="padding:0.75rem 1.25rem;background:var(--color-surface,#fafafa);"><button style="padding:0.4em 1em;border-radius:0.375rem;border:none;background:var(--color-primary,#6d28d9);color:#fff;font-weight:600;cursor:pointer;font-size:0.875rem;">Buy Now — $29.99</button></div>
</div>`}
...
2. **Add this component:** `/kit-add Card` — copies `card.tsx` + `card.scss` into your components directory (default `src/components/fpkit/`). All four sub-components ship in a single `card.tsx`. Or run `/kit-sync` to install all components at once.

</Steps>

Card has no upstream dependencies — it is self-contained.
...
Sub-components: `Card.Title` (`as` defaults to `h3`), `Card.Content` (`as` defaults to `article`), `Card.Footer` (`as` defaults to `div`). Each accepts `children` and native attributes for its element.
...
- [`component-card` skill](/acss-plugins-docs/acss-kit/skills/component-card) — maintainer reference (generation contract)
