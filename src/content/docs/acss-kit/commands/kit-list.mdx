...
Without a component name, `/kit-list` prints every available component organized by category. Components whose static-HTML output has been **Verified** (i.e. `/kit-add --target=html` can generate them) are marked `[HTML]`:
...
Components without `[HTML]` exist as React only — `/kit-add --target=html` will warn if you try to generate them.
