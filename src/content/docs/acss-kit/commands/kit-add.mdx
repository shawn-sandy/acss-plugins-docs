...
When you run `/kit-add`, the skill routes to the dedicated per-component skill at `skills/component-<name>/SKILL.md` for the requested component. Component names are normalized to lowercase before the path is resolved.

<Steps>

1. **Init check** — verifies that `sass` is in `devDependencies` and that `ui.tsx` exists in the target directory. On first run, also handles the foundation CSS install matrix (see [Foundation CSS install](#foundation-css-install) below).

2. **Lookup** — finds the component skill at `skills/component-<name>/SKILL.md`. If no matching skill exists, the command stops and lists available components.

3. **Dependency tree** — reads the component's Generation Contract to identify all dependencies, then recursively resolves them.
...
Run `/kit-list` for the full catalogue. As of acss-kit v1.2.0, each component has its own dedicated skill at `skills/component-<name>/`. Quick reference:
...
| Form        | `field`, `input`                                         |
| Layout      | `card`, `alert`, `nav`                                   |
| Complex     | `dialog`, `popover`, `table`                             |
| Inline only | `badge`, `tag`, `heading`, `text`, `details`, `progress` |

Inline-only components (Badge, Tag, etc.) live in `kit-core/references/inline-components.md` and are not supported by `/kit-create`. Use `/kit-add` to include them.
