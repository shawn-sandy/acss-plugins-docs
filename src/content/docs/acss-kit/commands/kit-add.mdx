...
1. **Lookup** — find the component skill at `skills/component-<name>/SKILL.md`. Normalize the component name to lowercase before resolving the path.

2. **Init check** — verify `sass` is in `devDependencies` and that `ui.tsx` exists in the target directory. If `ui.tsx` is missing, run the foundation initialization from `kit-core` (see [Foundation CSS install](#foundation-css-install) below).
