...
Starting with `acss-kit` v0.11.0, a vendored `foundation.css` (CSS reset, base typography, and `@layer` cascade ordering) ships alongside `ui.tsx`. The skill applies the following install matrix on first run:

| Project state                                                | Action                                                                                     |
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------ |
| No `ui.tsx` + no `foundation.css` (fresh install)            | Copy both `ui.tsx` and `foundation.css` plus the `sass/` source tree; print an import hint |
| `ui.tsx` present, `foundation.css` absent (existing install) | Prompt before copying — you can opt out to avoid the foundation layer                      |
| Both `ui.tsx` and `foundation.css` present                   | Skip silently — nothing to do                                                              |
