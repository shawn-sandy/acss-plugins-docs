...
When you run `/kit-add`, the command routes to the dedicated `component-<name>` skill for each requested component, which executes these steps:

<Steps>

1. **Init check** — verifies `sass` is in `devDependencies` and `ui.tsx` exists in the target directory. If `ui.tsx` is absent, runs the first-run initialization to copy it and `foundation.css` (see [Foundation CSS install](#foundation-css-install) below).

2. **Read the component reference** — loads `skills/component-<name>/reference.md` for the canonical templates, Generation Contract, Props Interface, CSS Variables, and Accessibility requirements.

3. **Resolve dependency tree** — reads the Generation Contract's `dependencies:` list and recursively resolves all dependencies. For example, `dialog` → `button`; `checkbox` → `input`.
