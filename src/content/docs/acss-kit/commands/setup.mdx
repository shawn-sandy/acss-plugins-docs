...
The skill executes nine sequential steps (including Step 6.5). Every step is **idempotent** — re-running `/setup` is safe and never overwrites existing files.
...
7. **Write `.browserslistrc`** — Writes `baseline widely available` (skipped when the project already declares a browser target in `package.json`, `.browserslistrc`, or a plain `browserslist` file).
8. **Seed a starter theme** — Generates `light.css` and `dark.css` from a hex seed (skipped with `--no-theme`).
9. **Wire theme imports** — Detects the CSS/SCSS entry point and adds idempotent `@import` statements.
