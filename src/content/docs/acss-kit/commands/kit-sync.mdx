...
2. **Enumerate the catalog** — glob `skills/component-*/SKILL.md` to find every component with a dedicated per-component skill. Also reads `kit-core/references/inline-components.md` for inline-only entries (Badge, Tag, Heading, Text/Paragraph, Details, Progress). Excludes `Form` and the `ui.tsx` foundation.
