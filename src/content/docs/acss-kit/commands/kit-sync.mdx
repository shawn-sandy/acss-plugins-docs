...
<Aside type="note">
  `Form` is **not** vendored by `/kit-sync` — it lives behind the
  `component-form` skill and is generated on-demand from a natural-language
  prompt. Run [`/setup`](/acss-plugins-docs/acss-kit/commands/setup) plus a
  form-shaped prompt (e.g. *"create a signup form with email and password"*) to
  bootstrap it.
</Aside>
...
2. **Enumerate the catalog** — glob `skills/component-*/SKILL.md` to discover every per-component skill, then read `kit-core/references/inline-components.md` to include the inline-only entries (Badge, Tag, Heading, Text/Paragraph, Details, Progress). Excludes `Form` and the `ui.tsx` foundation.
...
The inline-only components vendored in step 2 (Badge, Tag, Heading, Text/Paragraph, Details, Progress) are lightweight HTML primitives that render without a dedicated TSX file:

<ComponentPreview
  code={`<span class="badge">Badge</span>
...
<Aside type="tip">
  For step-by-step semantics (manifest schema, hash normalization rules,
  `--dry-run` behaviour, the interplay between `/kit-sync` and `/kit-update`),
  see the [`kit-sync` skill](/acss-plugins-docs/acss-kit/skills/kit-sync).
</Aside>
