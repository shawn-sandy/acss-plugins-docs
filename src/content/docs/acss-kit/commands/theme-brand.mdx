...
| `--from` | No       | Optional seed hex color. If provided, derives brand-specific primary values.                 |
...
:root {
  --color-primary: oklch(48% 0.2 155); /* forest green */
  --color-primary-hover: oklch(42% 0.2 155);
  --color-focus-ring: oklch(58% 0.18 155);
  --color-brand-accent: oklch(55% 0.16 80); /* warm amber accent */
}
...
[data-theme="dark"] {
  ...
  --color-brand-accent: oklch(72% 0.14 80);
}
```
...
<Aside type="caution">
  If you import the brand file before `light.css` or `dark.css`, the brand
  overrides will be overwritten by the base theme values.
</Aside>
...
Brand files are WCAG-validated after generation. The same 12 contrast pairs checked by `/theme-create` are re-evaluated with the overridden roles substituted in.
