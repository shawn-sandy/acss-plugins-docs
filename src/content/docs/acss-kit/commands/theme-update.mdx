...
3. Run WCAG validation on the temp copy against all 12 contrast pairs
...
<Aside type="caution">
  If you're updating both `light.css` and `dark.css`, run two separate
  `/theme-update` commands. Each file is validated independently.
</Aside>
...
Pre-validation failed. No changes written.
  ...
  --color-danger: #ff6b6b
    Contrast (danger / background): 2.8:1  ✗  (minimum 3.0:1)
...
<Aside type="tip">
  To see all current role values in a theme file before updating, view the file
  directly. The CSS custom properties are human-readable and sorted by semantic
  category.
</Aside>
