...
5. **WCAG validate** — checks all 12 contrast pairs.
...
<Aside type="note">
  Figma extraction reads color styles, component fills, and local variables —
  not pixel data. This means it works reliably even on complex designs with many
  layers.
</Aside>
...
<Aside type="tip">
  If the extracted color produces poor contrast ratios (e.g., a very light brand
  color against a white background), Claude will suggest an adjusted seed that
  preserves the hue while boosting contrast.
</Aside>
