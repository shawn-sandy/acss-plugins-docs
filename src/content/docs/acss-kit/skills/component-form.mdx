...
<Aside type="note">
  ...
  trigger vocabulary is what's being refined.
</Aside>

## Step 0 — Exit plan mode

If the session is in plan mode, the skill calls `ExitPlanMode` before resolving the field list. The skill may delegate to `/kit-add field input checkbox` (which writes TSX/SCSS), Step C writes the form component file, and `detect_target.py` runs via Bash — plan mode blocks all of it.

The skill stays in plan mode only when the user explicitly asked for a preview ("show me the field list first", "don't write the form yet"). In that case it narrates the resolved field list and the file that would be generated, then waits for approval — it does not invoke Write/Edit/Bash from a plan-mode session.
...
| file, upload, attachment                 | `Input` (file)                 |
