...
<Aside type="note">
  ...
  pattern. The generated output was stable; the natural language trigger
  vocabulary was what was being refined. It was planned to graduate to v1 once
  auto-trigger reliability was observed across a full release cycle on real user
  prompts with the v1 field-type grammar (text, email, password, tel, url,
  number, date, textarea, select, checkbox, radio). Failure-mode fallbacks for
  unsupported field types (file, color, range) are documented in the plugin's
