---
title: component-form skill (pilot)
description: Auto-generates accessible form components from natural language — no slash command required.
---
...
<Aside type="caution">
  The `component-form` pilot skill was **absorbed** into the [`components`
  skill](/acss-plugins-docs/acss-kit/skills/components) in acss-kit v1.0.0.
  **Form Mode** now lives in the `## Form Mode` section of
  `skills/components/SKILL.md`. This page is preserved for reference.
</Aside>

<Aside type="note">
  This was a **pilot skill** — it validated a per-component skill discovery
  pattern. The generated output is stable; the natural language trigger
  vocabulary is what's being refined. Graduates to v1 once auto-trigger
