---
title: component-form skill (absorbed)
description: Auto-generates accessible form components from natural language — now built into the components skill as Form Mode (acss-kit v1.0.0).
---
...
<Aside type="caution">
  **This skill was absorbed into the `components` skill as of acss-kit v1.0.0.**
  The `component-form` skill directory no longer exists upstream. Form Mode is
  now the `## Form Mode` section of the [`components`
  skill](/acss-plugins-docs/acss-kit/skills/components). This page is preserved
  for historical reference.
</Aside>

`component-form` is a pilot skill that auto-triggers on natural language form requests. Instead of running `/kit-add form`, you describe the form you want and the skill resolves the right field types, generates the dependencies, and produces a complete, accessible form component.
