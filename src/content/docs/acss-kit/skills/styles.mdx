...
The `styles` skill drives all theme-related commands. It defines the OKLCH palette math, the 18 semantic color role catalogue, the WCAG contrast pair validation contract, the file format for theme output, and the DESIGN.md / Figma variable adapter pipelines.

## Trigger

Triggered by `/theme-create`, `/theme-brand`, `/theme-extract`, `/theme-update`, `/theme-from-design`, `/theme-from-figma`, and `/design-export`. Also invoked by the `style-tune` pilot skill when adjusting theme-layer tokens.
...
4. **Text:**
   ...
   ```
   --color-text:        L 10%  (near-black)
   --color-text-inverse:#ffffff (white — text on colored surfaces)
   --color-text-muted:  lightest L with 4.5:1 vs background (≈ L 38–48%)
   ```
...
| Role                   | Light L | Dark L | Usage                       |
| ---------------------- | ------- | ------ | --------------------------- |
| `--color-text`         | 10%     | 96%    | Body text                   |
| `--color-text-inverse` | white   | 12%    | Text on colored backgrounds |
| `--color-text-muted`   | ~38–48% | 78%    | Secondary / captions        |
...
### Optional roles (set via DESIGN.md import or manual authoring)

These three roles are recognized by the token pipeline (`_tokens.py` `ROLE_GROUPS`) but are not emitted by the palette generator — they must be set from a DESIGN.md import (`/theme-from-design`, `/theme-from-figma`) or added manually to `light.css` / `dark.css`.

| Role                     | Usage                                              |
| ------------------------ | -------------------------------------------------- |
| `--color-surface-subtle` | Table-stripe / hover surface (optional background) |
...
The skill validates 12 contrast pairs (`validate_theme.py:PAIRS`). Pairs where either role is absent in the file are silently skipped (relevant for optional roles).

| Pair                            | Minimum | Category                      |
| ------------------------------- | ------- | ----------------------------- |
| `text` / `background`           | 4.5:1   | Text                          |
| `primary` / `background`        | 3.0:1   | Non-text (interactive target) |
| `text-inverse` / `primary`      | 4.5:1   | Text on brand                 |
| `text-muted` / `background`     | 4.5:1   | Text                          |
| `text` / `surface`              | 4.5:1   | Text on card                  |
| `success` / `background`        | 3.0:1   | Non-text indicator            |
| `warning` / `background`        | 3.0:1   | Non-text indicator            |
| `danger` / `background`         | 3.0:1   | Non-text indicator            |
| `info` / `background`           | 3.0:1   | Non-text indicator            |
| `focus-ring` / `background`     | 3.0:1   | WCAG 1.4.11 focus indicator   |
| `focus-ring` / `surface`        | 3.0:1   | WCAG 1.4.11 focus indicator   |
| `focus-ring` / `surface-raised` | 3.0:1   | WCAG 1.4.11 focus indicator   |
