...
Applies to every flow (`/theme-create`, `/theme-brand`, `/theme-update`, `/theme-extract`). If the session is in plan mode, the skill calls `ExitPlanMode` before proceeding — all four flows write theme CSS and shell out to a per-flow subset of `generate_palette.py`, `tokens_to_css.py`, `validate_theme.py`, `css_to_tokens.py`, and `verify_integration.py`. Plan mode blocks every one of those Write/Edit/Bash calls.

The skill stays in plan mode only when the user explicitly asked for a preview ("show me the palette first", "don't write the theme yet"). In that case it narrates which roles would be generated and which files would be written — without invoking Write/Edit/Bash — and waits for approval.
