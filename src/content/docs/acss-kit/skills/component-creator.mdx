...
The `component-creator` skill powered [`/kit-create`](/acss-plugins-docs/acss-kit/commands/kit-create). It also auto-triggered on natural-language phrases. `/kit-create` is now handled by the [`components` skill](/acss-plugins-docs/acss-kit/skills/components) (Creator Mode).

<Aside type="note">
  The `component-creator` pilot skill was absorbed into the [`components`
  skill](/acss-plugins-docs/acss-kit/skills/components) in acss-kit v1.0.0.
  **Creator Mode** now lives in the `## Creator Mode` section of
  `skills/components/SKILL.md`. This page is preserved for reference.
</Aside>
