...
The `components` skill drives `/kit-add`. It defines the seven-step generation workflow, the component reference system, and all the conventions that keep generated code consistent across every component.

## Trigger

The `components` skill is triggered by `/kit-add`, `/kit-create`, and natural-language phrases. It handles three modes:

- **Standard Mode** — `/kit-add` generates one or more components with full dependency resolution (Steps 0, A–F below).
- **Creator Mode** — `/kit-create` or natural language ("create a primary pill button") generates a single component instance from a description.
- **Form Mode** — natural language ("build a signup form") auto-generates an accessible form component from field-type grammar.
