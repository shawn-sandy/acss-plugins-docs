...
description: Full component generation workflow — React (Steps A–F), Creator Mode (/kit-create), Form Mode, and HTML Target (/kit-add --target=html).
---

import { Aside, Steps } from "@astrojs/starlight/components";

The `components` skill (v0.4.0) drives all component generation for acss-kit. It contains four integrated workflows: the standard React generation pipeline (Steps A–F), **Creator Mode** for natural-language descriptions, **Form Mode** for accessible form scaffolding, and **HTML Target** for static HTML + SCSS + vanilla-JS output.

<Aside type="note">
  As of acss-kit v1.0.0, the former `component-creator`, `component-form`, and
  `components-html` pilot skills have been absorbed into this single skill. The
  separate skill directories no longer exist upstream. See the corresponding
  [component-creator](/acss-plugins-docs/acss-kit/skills/component-creator),
  [component-form](/acss-plugins-docs/acss-kit/skills/component-form), and
  [components-html](/acss-plugins-docs/acss-kit/skills/components-html) pages
  for historical reference; they are preserved here but are no longer active
  separate skills upstream.
</Aside>
...
| Trigger                                                                     | Workflow                                       |
| --------------------------------------------------------------------------- | ---------------------------------------------- |
| `/kit-add <component>`                                                      | Standard React generation (Steps A–F)          |
| `/kit-add --target=html <component>` or `/kit-add-html`                     | HTML Target workflow (steps HT-A through HT-F) |
| `/kit-create <description>` or natural language ("create a…", "make me a…") | Creator Mode                                   |
| Natural language ("create a signup form", "build a contact form with…")     | Form Mode                                      |
