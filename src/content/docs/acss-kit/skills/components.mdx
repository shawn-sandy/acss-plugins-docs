...
3. **Step B — Verify sass and copy `ui.tsx`.** Checks `devDependencies` in `package.json` for `sass`. If missing, halts with an install command. Then checks whether `ui.tsx` exists in the components directory — if not, copies `assets/foundation/ui.tsx` there. The foundation file is never overwritten on subsequent runs.
