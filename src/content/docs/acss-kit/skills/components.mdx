---
title: components skill (legacy)
description: The monolithic components skill that drove /kit-add and /kit-create. Renamed to kit-core in acss-kit v1.2.0; per-component generation now lives in dedicated component-<name> skills.
---

import { Aside, Steps } from "@astrojs/starlight/components";

<Aside type="caution">
  The `components` skill was **renamed to `kit-core`** in acss-kit v1.2.0. Per-component
  generation has moved to fifteen dedicated `component-<name>` skills. The
  [`kit-core skill`](/acss-plugins-docs/acss-kit/skills/kit-core) page documents
  the current orchestrator. This page is preserved for reference.
</Aside>

The `components` skill drove `/kit-add` and `/kit-create`. It defined the seven-step generation workflow, the component reference system, and all the conventions that keep generated code consistent across every component.

## Trigger

The `components` skill was triggered by `/kit-add` and `/kit-create`. It provided both the standard generation workflow (Steps 0, A–F) and the **Creator Mode** and **Form Mode** sections that were previously handled by the separate `component-creator` and `component-form` pilot skills (absorbed in v1.0.0).

In v1.2.0, `/kit-add <component>` now routes to `skills/component-<name>/SKILL.md` directly. The `kit-core` skill handles orchestration for `/kit-create`, `/kit-list`, `/kit-sync`, `/kit-update`, and Form/HTML/Style-Tune modes.
...
2. **Step A — Resolve component name.** The skill looks up the requested name in the component reference catalogue (previously `skills/components/references/catalog.md`; removed in v1.2.0 when each component moved to its own `skills/component-<name>/` directory). Names are normalized (case-insensitive, hyphenated or camelCase both accepted). If a name doesn't match, the skill lists the closest alternatives and stops.
...
In v1.0.0–v1.1.x, each component had a spec doc at `skills/components/references/components/<name>.md`. In v1.2.0, these moved to per-component skill directories: `skills/component-<name>/reference.md` (one directory per component). The spec sections are the same:
