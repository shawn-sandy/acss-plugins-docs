...
3. **Step B — Verify sass and copy `ui.tsx` and `foundation.css`.** Checks `devDependencies` in `package.json` for `sass`. If missing, halts with an install command. Then applies the three-case foundation install matrix (see [Foundation CSS install](#foundation-css-install) below).
