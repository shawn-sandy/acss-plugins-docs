...
| **Plan mode** | Exits immediately so file writes can run |
...
| 7.5 | Wire theme imports into the project's CSS/SCSS entry point | — |
| 8 | Print summary listing `created[]` vs `kept[]` | — |
...
src/styles/main.scss             # @import statements added idempotently
```
...
<Aside type="note">
The skill never deletes files. If a previous bootstrap left partial state, a re-run will fill in only what's missing.
</Aside>
