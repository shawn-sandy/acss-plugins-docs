...
<Aside type="tip">
For React-only projects, use the [`components` skill](/acss-plugins-docs/acss-kit/skills/components) (driven by [`/kit-add`](/acss-plugins-docs/acss-kit/commands/kit-add)) instead. The two are designed to coexist — the framework-agnostic SCSS contract is the foundation that makes both possible.
</Aside>
