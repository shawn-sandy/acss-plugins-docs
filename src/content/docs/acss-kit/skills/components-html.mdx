...
<Aside type="caution">
  The `components-html` skill was **removed** upstream in acss-kit v1.0.0. The
  HTML Target workflow is now part of the [`components`
  skill](/acss-plugins-docs/acss-kit/skills/components) (`## HTML Target`
  section, steps HT-A through HT-F), accessed via `/kit-add --target=html`. This
  page is preserved for reference; use `/kit-add --target=html` going forward.
</Aside>

The `components-html` skill drove [`/kit-add-html`](/acss-plugins-docs/acss-kit/commands/kit-add-html). It was a sibling to the [`components` skill](/acss-plugins-docs/acss-kit/skills/components) — both read the same reference docs at `references/components/<name>.md`. The TSX generator copies the `## TSX Template` block; this generator copied `## HTML Template` and (for stateful components) `## Vanilla JS`. The `## SCSS Template` block is identical for both — the SCSS is framework-agnostic.
