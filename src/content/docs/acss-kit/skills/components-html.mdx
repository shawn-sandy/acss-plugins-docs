---
title: components-html skill (absorbed)
description: Generate static HTML snippets, SCSS, and vanilla-JS helpers — now the HTML Target section of the components skill (acss-kit v1.0.0).
---
...
<Aside type="caution">
  **This skill was absorbed into the `components` skill as of acss-kit v1.0.0.**
  The `components-html` skill directory no longer exists upstream. HTML
  component generation is now the `## HTML Target` section (steps HT-A through
  HT-F) of the [`components`
  skill](/acss-plugins-docs/acss-kit/skills/components), triggered by `/kit-add
  --target=html`. This page is preserved for historical reference.
</Aside>

The `components-html` skill previously drove [`/kit-add-html`](/acss-plugins-docs/acss-kit/commands/kit-add-html). It was a sibling to the [`components` skill](/acss-plugins-docs/acss-kit/skills/components) — both read the same reference docs at `references/components/<name>.md`. The TSX generator copied the `## TSX Template` block; the HTML generator copied `## HTML Template` and (for stateful components) `## Vanilla JS`. The `## SCSS Template` block is identical for both — the SCSS is framework-agnostic.
