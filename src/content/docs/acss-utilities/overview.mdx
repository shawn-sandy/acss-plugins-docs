...
<Aside type="caution">
  **`acss-utilities` is deprecated (tombstoned at v1.0.0).** All utility-class
  commands (`/utility-add`, `/utility-bridge`, `/utility-list`, `/utility-tune`)
  have moved to **acss-kit v1.0.0**. Existing installs keep working, but no new
...
See the [Migration Guide](/acss-plugins-docs/acss-kit/migration-v1) for the full
upgrade steps. No CSS class names changed — your existing `utilities.css` and
`token-bridge.css` files work without modification.

</Aside>

**acss-utilities** provided a pre-built bundle of Tailwind-style atomic CSS utility classes that connect to your acss-kit theme via a token bridge. This plugin is now deprecated; the same commands and bundle are available in **acss-kit v1.0.0**.
