...
<Aside type="danger">
  **`acss-utilities` has been fully removed from upstream as of acss-kit
  v1.1.0 (2026-05-21).** The plugin directory (`plugins/acss-utilities/`) was
  deleted from the agentic-acss-plugins repository. All utility commands
  (`/utility-add`, `/utility-bridge`, `/utility-list`, `/utility-tune`) and the
  `utilities` skill now live exclusively in **acss-kit**. This docs section is
...
plugin was tombstoned in acss-kit v1.0.0 and fully removed from upstream in
v1.1.0. This section is preserved for historical reference; all functionality
is now in acss-kit.
