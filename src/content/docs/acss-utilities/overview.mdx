...
<Aside type="caution">
  **`acss-utilities` is deprecated.** All utility-class commands and the
  `utilities` skill have moved to **acss-kit v1.0.0**. Existing installs
  continue to work, but no new features will be added.
...
See the [Migration Guide](/acss-plugins-docs/acss-kit/migration-v1) for
full upgrade steps. No CSS class names changed — your `utilities.css` and
`token-bridge.css` files work without modification.

</Aside>

**acss-utilities** provides a pre-built bundle of Tailwind-style atomic CSS utility classes that connect to your acss-kit theme via a token bridge. This page is preserved for reference; the canonical documentation is now under [acss-kit](/acss-plugins-docs/acss-kit/overview).
