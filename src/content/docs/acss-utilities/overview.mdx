...
<Aside type="caution" title="Plugin tombstoned — acss-utilities v1.0.0">
  All utility-class commands (`/utility-add`, `/utility-bridge`, `/utility-list`,
  `/utility-tune`) moved to **acss-kit v1.0.0**. No new features will be added
  to the `acss-utilities` plugin. Existing installs continue to work; no CSS
  class names changed.

**Migrate:** See the [Migration Guide](/acss-plugins-docs/acss-kit/migration-v1)
for the two-command upgrade path. After migrating, use the
[acss-kit utility commands](/acss-plugins-docs/acss-kit/overview#utility-commands)
going forward.
...
<UtilityExample class="md-flex-row" property="flex-direction" />
...
<ColorSwatch roles={["primary", "error", "surface"]} />
