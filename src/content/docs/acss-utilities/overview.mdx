...
<UtilityExample class="bg-primary" property="--color-primary">
  Sample element styled with <code>.bg-primary</code> — color comes from the
  token bridge.
</UtilityExample>

## Responsive variants

Four families support responsive prefixes (`spacing`, `display`, `flex`, `grid`). The prefix activates the utility at that breakpoint and above:
...
<UtilityExample class="md-flex-row" property="flex-direction">
  Responsive flex: column on mobile, row from <code>md</code> breakpoint up.
</UtilityExample>
