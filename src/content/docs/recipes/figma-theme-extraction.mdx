...
```
Light mode — 12 contrast pairs:  ✓ all pass
Dark mode  — 12 contrast pairs:  ✓ all pass
...
<Aside type="tip">
  For a closer match to Figma's exact colors, use `/theme-update` to override
  specific roles after extraction rather than re-running `/theme-extract`.
  Extraction picks the "best contrast-passing" version of your Figma color —
  which may differ slightly from the source value.
</Aside>
