...
2. Review the WCAG output. All 12 pairs should pass at 4.5:1 or 3.0:1:

   ```
   Light mode — all 12 contrast pairs:  ✓
   Dark mode  — all 12 contrast pairs:  ✓
...
<Aside type="note">
  `aria-pressed` on the toggle button communicates the current state to screen
  readers. "Button, dark mode, pressed" in light mode; "Button, dark mode, not
  pressed" in dark mode.
</Aside>
